...
class CATransBlock(nn.Module):
    def __init__(self, conv_dim, drop_conv, act_conv, ca_expand, is_group, is_bias,
                 trans_dim, head_dim, window_size, drop_trans, win_type, input_resolution=None, 
                 act_trans='gelu', group_shuffle=16):
        super(CATransBlock, self).__init__()
        
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.group = group_shuffle
        ...
        self.local_block = LocalBlock(trans_dim, drop_path=drop_conv, 
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        trans_x = self.conv_head(x)
        conv_x = self.local_block(trans_x )
        ...
        # channel shuffle
        res = conv_x + trans_x
        res = Rearrange('b (g c) h w -> b g c (h w)', g = self.group)(res)
        res = Rearrange('b g c s -> b c g s')(res)
        res = Rearrange('b c g (h w) -> b (c g) h w', h=h, w=w)(res)
        x = x + self.conv_tail(res)
        return x
    
    
class ResidualGroup(nn.Module):
    def __init__(self, num_blk, conv_dim, drop_conv, act_conv, ca_expand, is_group, is_bias,
                 trans_dim, head_dim, window_size, drop_trans, input_resolution=None, 
                 act_trans='gelu', group_shuffle=4):
        ...
        module_body = [
            CATransBlock(conv_dim, drop_conv=drop_conv[i], act_conv=act_conv, ca_expand=ca_expand,
                         ...
                         input_resolution=input_resolution, act_trans=act_trans, group_shuffle=group_shuffle)
...
class BaseAddShuffle(nn.Module):
    def __init__(self, in_ch, conv_dim, drop_conv, act_conv, num_groups, ca_expand, is_group, is_bias,
                 trans_dim, head_dim, window_size, drop_trans, input_resolution=None, act_trans='gelu', group_shuffle=16):
        super(BaseAddShuffle, self).__init__()
